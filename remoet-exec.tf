# it create new ssh key pear(aws_key) in aws make sure u have pravite key with you or u can pass your your existing key name 
...
resource "aws_instance" "ec2_example" {
  ...
  provisioner "remote-exec" {         # after making connection it will run this script or cmd like how we use user data
    inline = [
      "touch hello.txt",
      "echo helloworld remote provisioner >> hello.txt",
    ]
  }
  connection {              # it will make connection to ec2 
      type        = "ssh"
      host        = self.public_ip
      user        = "ubuntu"
      private_key = file("/home/rahul/Jhooq/keys/aws/aws_key")        # path and name of key file 
      timeout     = "4m"
   }
}
